...
export default async function handler(req, res) {
  ...
  try {
    ...
    const project = (process.env.OPENAI_PROJECT_ID || '').trim(); // needed for sk-proj- keys
    ...
    const r = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        ...(prevId ? { previous_response_id: prevId } : {}), // keep the conversation going
      }),
    });
    ...
    return res.status(200).json({ reply, responseId: data?.id });
  } catch (e) {
